...
from graph_reasoning.GNNs import RGAT
...
class RobertaGraphReasoning(RobertaPreTrainedModel):
    def __init__(self, config):
        ...
        self.output_layer1 = nn.Linear(config.hidden_size * 3 + Config.model_args.gnn_layers_num,
                                       config.hidden_size * 2)
        self.output_layer2 = nn.Linear(config.hidden_size * 2, 1)
        self.reasoning_linear = nn.Linear(config.hidden_size * 2, Config.label_num)
    ...
    def _get_split_origin_context_answer_representation(self, input_ids, last_hidden_states):
        ...
        sep_interval = [[(1, sep_locs[index][0]), (sep_locs[index][0] + 1, sep_locs[index][1])] for index in range(len(sep_locs))]
        ...
        #                                    index, si in enumerate(sep_interval)]
        answer_origin_representations = [torch.mean(last_hidden_states[index, si[1][0]:si[1][1], :], dim=0) for
                                         index, si in enumerate(sep_interval)]
        return torch.stack(context_origin_representations), torch.stack(answer_origin_representations)
    ...
    def forward(
        ...
        context_origin_representations, answer_origin_representations = self._get_split_origin_context_answer_representation(
        ...
        outputs = self.output_layer1(
            torch.cat([context_origin_representations, answer_origin_representations, graphs_representations], dim=-1))
        outputs = torch.tanh(outputs)
        outputs = self.reasoning_linear(outputs)
